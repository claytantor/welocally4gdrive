...
(function( $ ){
	  ...
	  $.fn.searchGrid = function( cfg ) {  
		    return this.each(function() {
		    	
		    	var $this = $(this);
		    	$this.cfg = cfg;
		    	
		    	var grid = new WELOCALLY_GeodbRecordsGrid($this,{
		    		
		    	});
		    	
		    	var geodbSearch = new WELOCALLY_GeodbRecordSearch({
		    		query: cfg.query,
		    		observers: [grid]
		    	});
		  ...
		  $.fn.searchMap = function( cfg ) {  
			    return this.each(function() {
			    	...
			    	var recordSelected = new WELOCALLY_GeodbRecord();

			    	//the multi map
			    	var recordsMap = 
			    		  new WELOCALLY_GeodbRecordsMap({ 
			    			id:'multi_googledocs_1',
			    			imagePath: cfg.baseUrl+'images/marker_all_base.png',
			    			showSelection: false,
			    			showLetters: true,
			    			overrideSelectableStyle: 'width: 100px;',
			    			observers:[recordSelected]		
			    	     }).init(); 

			    	//setup the selected area   
			    	recordSelected.initCfg(cfg);
			    	recordsMap.getSelectedArea().append(recordSelected.makeWrapper());
			    	
			    	var geodbSearch = new WELOCALLY_GeodbRecordSearch({
			    		query: cfg.query,
			    		observers: [recordsMap]
			    	});
			    			    	
			    	geodbSearch.search();
			    	$this.html(recordsMap.makeWrapper());
