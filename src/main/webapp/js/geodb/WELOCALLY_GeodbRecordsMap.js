...
function WELOCALLY_GeodbRecordsMap (cfg) {	
	...
	this.init = function() {
		...
		if(errors){
			...
			WELOCALLY_Util.ui.setStatus(errorsArea, errors,'wl_error');
...
WELOCALLY_GeodbRecordsMap.prototype.initCfg = function(cfg) {
	var errors = [];
	if (!cfg) {
	...
	if (!cfg.id) {
		cfg.id = WELOCALLY_Util.util.keyGenerator();
	}
...
WELOCALLY_GeodbRecordsMap.prototype.makeWrapper = function() {
	// Build wrapper
	var _instance = this;
...
WELOCALLY_GeodbRecordsMap.prototype.setRecords = function(data) {
	var _instance = this;
	
	if(_instance.map == null){
		_instance.map = _instance.initMapForRecords(data, _instance.map_canvas, _instance.recordMarkers);
	} else {
		_instance.deleteOverlays(_instance.recordMarkers);
	    _instance.addPlaces(_instance.map, data, _instance.recordMarkers);
	    _instance.setMapEvents(_instance.map, _instance.recordMarkers);		
	}
...
WELOCALLY_GeodbRecordsMap.prototype.initMap = function(map_canvas) {
...
WELOCALLY_GeodbRecordsMap.prototype.initMapForRecords = function(data, map_canvas, recordMarkers) {
    ...
    //add all the markers
    _instance.addPlaces(map, data, recordMarkers);
           
    _instance.setMapEvents(map, recordMarkers);
...
WELOCALLY_GeodbRecordsMap.prototype.initMapForPlaces = function(places, map_canvas, recordMarkers) {
    ...
    //add all the markers
    _instance.addPlaces(map, places, recordMarkers);
...
WELOCALLY_GeodbRecordsMap.prototype.makeInfoBox = function(boxText){
...
WELOCALLY_GeodbRecordsMap.prototype.addRecords = function(map, data, markers){
	var _instance = this;
	...
	$(_instance.results).empty();
	$(_instance.results).hide();
	_instance.infoBox.close();
	
	$(data).each(function (findex, nv) {
...
WELOCALLY_GeodbRecordsMap.prototype.addPlaces = function(map, places, recordMarkers){
...
WELOCALLY_GeodbRecordsMap.prototype.makeMinimumBounds= function(bounds){
	var _instance = this;
...
WELOCALLY_GeodbRecordsMap.prototype.makeItemContents = function (item, i, showMarker) {
...
WELOCALLY_GeodbRecordsMap.prototype.setMapEvents = function(map, markers){
	...
	google.maps.event.addListener(map, 'zoom_changed', function() {
		zoomChangeBoundsListener = google.maps.event.addListener(map, 'bounds_changed', function(event) {
			...
			_instance.setStatus(
					_instance.mapStatus, 
					_instance.makeMapStatus(_instance.map, _instance.recordMarkers) , 
					'wl_message', false);
	...
	var tilesHandle = google.maps.event.addListener(map, 'tilesloaded', function() {
		...
		WELOCALLY_Util.util.preload([
				 'https://maps.google.com/mapfiles/openhand.cur'
		]);          				
	}); 
	
},

WELOCALLY_GeodbRecordsMap.prototype.selectedItemHandler = function(event, ui) {
	...
	if(this.nodeName=='OL'){
		...
		marker = _instance.recordMarkers[index];		
		_instance.selectedPlace = 
			_instance.recordMarkers[index].item;
...
WELOCALLY_GeodbRecordsMap.prototype.resetOverlays=function (location, markersArray) {
	...
	_instance.recordMarkers = [];

};

WELOCALLY_GeodbRecordsMap.prototype.refreshMap = function(searchLocation, bounds) {
	var _instance = this;
	google.maps.event.trigger(_instance.map, 'resize');
	
	if(_instance.recordMarkers.length==0){
		var markerIconLocation = 
			new google.maps.MarkerImage(_instance.cfg.imagePath, new google.maps.Size(32, 32), new google.maps.Point(0, 0));
...
WELOCALLY_GeodbRecordsMap.prototype.showOverlays = function (markersArray, map) {
	if (markersArray) {
...
WELOCALLY_GeodbRecordsMap.prototype.deleteOverlays=function (markersArray) {
  if (markersArray) {
	for (i in markersArray) {
...
WELOCALLY_GeodbRecordsMap.prototype.colName = function (n) {
...
WELOCALLY_GeodbRecordsMap.prototype.addMarker = function(i, markersArray, markerMap, location, item, icon) {
	var _instance = this;
...
WELOCALLY_GeodbRecordsMap.prototype.addMarkerCenter = function(markersArray, markerMap, location, icon) {
	var _instance = this;
	var marker = new google.maps.Marker({
	    instance: this,
		position: location,
...
WELOCALLY_GeodbRecordsMap.prototype.clearOverlays = function (markersArray) {
  if (markersArray) {
	for (i in markersArray) {
	  markersArray[i].setMap(null);
	}
...
WELOCALLY_GeodbRecordsMap.prototype.getMapRadius = function(map) {
...
WELOCALLY_GeodbRecordsMap.prototype.getBoundsDistance = function(bounds) {
	var _instance = this;
...
WELOCALLY_GeodbRecordsMap.prototype.setStatus = function(statusArea, message, type, showloading){
	var _instance  = this;
...
WELOCALLY_GeodbRecordsMap.prototype.makeMapStatus = function (map, markers) {	
...
WELOCALLY_GeodbRecordsMap.prototype.getSelectedArea = function (){
	var _instance = this;
	return _instance.selectedSection;
};

//use map status area
WELOCALLY_GeodbRecordsMap.prototype.getStatusArea = function (){
	var _instance = this;
	return _instance.mapStatus;
};

WELOCALLY_GeodbRecordsMap.prototype.triggerResize = function(){
...
//for place selectors
WELOCALLY_GeodbRecordsMap.prototype.show = function(selectedPlace) {	
